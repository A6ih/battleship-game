...
const getFinalCords = (placeShip: string[], filledCords: Set<string>, size: number) => {
     ...
     if(cordsArrNum[1] + size > 9) {
        return getFinalCords(placeShip, filledCords, size)
     }
...
export default class Computer {
    ...
    placeShips() {
        ...
        while(shipArr.length > 0) {
            ...
            const cords = getFinalCords(this.cordsToPlaceShip, this.gameboard.getFilledCords(), ship.size)
            this.gameboard.placeShip(ship.size, ship.name, cords)
        }
    }
}